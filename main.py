...
# Define your inputs
MP4_FILE = "./video.mp4"  # Base file to add subtitles to
CSV_FILE = "./translations.csv"  # CSV translation
LANGUAGE = "English"  # What language to add subtitles for (via csv)
SUB_POS = "bottom"  # Subtitle position
BOX_TOP = 0.15  # Letterbox top size %
BOX_BOT = 0.24  # Letterbox bottom size %
...
def process_video(mp4_path, csv_path, language):
    ...
    def letterbox_filter(frame):
        ...
        frame[-size_b:, :] = 0
        return frame
    letterboxed = CompositeVideoClip([video_clip]).fl_image(letterbox_filter)
